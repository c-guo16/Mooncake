...
class PinnedBuffer {
   public:
    explicit PinnedBuffer(size_t size) : size_(size), ptr_(nullptr) {
        hipError_t err = hipHostMalloc(&ptr_, size_);
        if (err != hipSuccess) {
...
void fillData(int thread_id, void *addr, uint8_t seed) {
    ...
    for (int i = 0; i < FLAGS_batch_size; ++i) {
        uint8_t *local_addr =
            (uint8_t *)(addr) +
            FLAGS_block_size * (i * FLAGS_threads + thread_id);
        hipMemcpyAsync(local_addr, ref_buf.data(), FLAGS_block_size,
                        hipMemcpyDefault, s);
    }
...
void checkData(int thread_id, void *addr, uint8_t seed) {
    ...
    for (int i = 0; i < FLAGS_batch_size; ++i) {
        ...
        hipStream_t s;
        hipStreamCreate(&s);
        hipMemcpyAsync(user_buf.data(), local_addr, FLAGS_block_size,
                        hipMemcpyDefault, s);
        hipStreamSynchronize(s);
        hipStreamDestroy(s);
        if (memcmp(user_buf.data(), ref_buf.data(), FLAGS_block_size) != 0) {
